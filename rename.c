/*
    Mengubah nama file
    contoh : 
    rename ‘s/.prog/.prg/’ *.prog -> mengganti nama semua file dengan ekstensi .prog menjadi .prg’
...
void rename_rek (char *from,char *ext1,char *ext2)
{
    //char buff[1024];
    int fd0;
    struct dirent de;
...
int main(int argc,char *argv[]){
	...
	if(argv[1][0]!='-') com=1;
        ...
        com=1;
        while(argv[com][0]=='-'){
		    if(argv[com][1]=='n') n=1; //-n = tidak melakukan perubahan apapun
		    if(argv[com][1]=='v') v=1; //-v = menunjukan file mana saja yang telah direname, apabila ada
            if(argv[com][1]=='h') help();
            if(argv[com][1]=='o') o=1; //-o = tidak overwrite file yang ada
            com++;
        }
	}
